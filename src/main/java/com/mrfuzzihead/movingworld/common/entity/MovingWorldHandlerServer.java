...
import com.mrfuzzihead.movingworld.common.network.ChunkBlockUpdateMessage;
...
public abstract class MovingWorldHandlerServer extends MovingWorldHandlerCommon {
    ...
    public boolean interact(EntityPlayer player) {
        if (getMovingWorld().riddenByEntity == null) {
            ...
            return getMovingWorld().getCapabilities()
                .mountEntity(player);
        }
    ...
    public void onChunkUpdate() {
        ...
        if (!firstChunkUpdate) {
            ...
            MovingWorld.instance.network.sendToAllAround(
                msg,
                new TargetPoint(
                    getMovingWorld().worldObj.provider.dimensionId,
                    getMovingWorld().posX,
                    getMovingWorld().posY,
                    getMovingWorld().posZ,
                    64D));
